...
def search_emails(
    ...
    logging.debug(f"Search found {len(formatted_results)} results.")
...
def read_email(message_id: str) -> Optional[Email]:
    ...
    # NOTE: オリジナルの art-e では message_id (文字列) で引いていたため
    # 宛先が常に空になるバグがあった。recipients.email_id は emails.id を
    # 参照する整数 FK なので、ここでは PK で引く。
    recipients_sql = """
